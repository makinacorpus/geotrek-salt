...
etc-init-supervisor.{{cfg.name}}:
  cmd.run:
    - name: >
            cp -fv
            {{cfg.project_root}}/geotrek/etc/init/supervisor.conf
...
{{cfg.name}}-service:
  ...
  cmd.run:
    ...
    - onlyif: test "$({{cfg.project_root}}/geotrek/bin/supervisorctl status 2>&1 |grep "refused connection"|wc -l)" != 0
    - user: root
    - watch:
...
{{cfg.name}}-reboot:
  file.managed:
    ...
    - contents: |
                ...
                {{ cfg.project_root}}/geotrek/bin/supervisorctl restart convertit
                {{ cfg.project_root}}/geotrek/bin/supervisorctl restart screamshotter
                {% endif %}
                {% if data.has_geotrek %}
                {{ cfg.project_root}}/geotrek/bin/supervisorctl restart geotrek
                {{ cfg.project_root}}/geotrek/bin/supervisorctl restart geotrek_api
                {{ cfg.project_root}}/geotrek/bin/supervisorctl restart tilecache
                {%- endif %}
  cmd.run:
...
etc-logrotate.{{cfg.name}}:
  cmd.run:
    - name: |
            cp {{cfg.project_root}}/geotrek/etc/logrotate.conf /etc/logrotate.d/{{cfg.name}}
